...
>(({ className, children, showBar = true, ...props }, ref) => (
  <DrawerPortal>
    ...
    <DrawerPrimitive.Content
      ref={ref}
      className={cn(
        "fixed inset-x-0 bottom-0 z-[3000] mt-24 flex h-auto flex-col rounded-t-[10px] border bg-background max-w-full",
        className
      )}
      {...props}
    >
...
const DrawerHeader = ({
  ...
  <div
    className={cn(
      "flex justify-between items-center gap-1.5 p-4 text-center sm:text-left",
      className
    )}
    {...props}
  />
